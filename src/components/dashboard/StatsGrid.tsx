...
export function StatsGrid({ stats }: StatsGridProps) {
  ...
  if (stats.ytd_run_totals && stats.ytd_run_totals.count > 0) {
    ytdStats.push({
      title: 'YTD Runs',
      stats: stats.ytd_run_totals,
      icon: '🏃',
    });
  }

  if (stats.ytd_swim_totals && stats.ytd_swim_totals.count > 0) {
    ytdStats.push({
      title: 'YTD Swims',
      stats: stats.ytd_swim_totals,
      icon: '🏊',
    });
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {ytdStats.length > 0 && (
        <div>
          <h2 className="text-2xl font-bold text-white mb-4">📈 Year to Date</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
